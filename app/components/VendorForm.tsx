...
export default function VendorForm({ initial, vendorId }: { initial?: any, vendorId?: string }) {
  ...
  async function handleSubmit(e: any) {
    ...
    if (!form.vendorName || !form.bankAccountNo || !form.bankName) {
      setError("Vendor Name, Bank Account No. and Bank Name are required"); return;
    }
    try {
      if (vendorId) {
  ...
  return (
    <form onSubmit={handleSubmit} style={{ maxWidth: 400 }}>
      ...
      <button type="submit" style={{ marginTop: 10 }}>{vendorId ? "Update" : "Create"}</button>
    </form>
  );
}
